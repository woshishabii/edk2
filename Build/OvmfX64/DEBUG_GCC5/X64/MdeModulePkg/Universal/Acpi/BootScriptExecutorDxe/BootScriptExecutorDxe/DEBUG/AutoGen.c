...
GLOBAL_REMOVE_IF_UNREFERENCED GUID gEfiCallerIdGuid = {0xFA20568B, 0x548B, 0x4b2b, {0x81, 0xEF, 0x1B, 0xA0, 0x8D, 0x4A, 0x3C, 0xEC}};
...
GLOBAL_REMOVE_IF_UNREFERENCED CHAR8 *gEfiCallerBaseName = "BootScriptExecutorDxe";
...
GLOBAL_REMOVE_IF_UNREFERENCED EFI_GUID gEfiLockBoxProtocolGuid = { 0xbd445d79, 0xb7ad, 0x4f04, { 0x9a, 0xd8, 0x29, 0xbd, 0x20, 0x40, 0xeb, 0x3c }};
...
#define _PCD_TOKEN_PcdSpinLockTimeout  184U
...
#define _PCD_TOKEN_PcdS3BootScriptTablePrivateDataPtr  27U
...
#define _PCD_TOKEN_PcdS3BootScriptTablePrivateSmmDataPtr  28U
...
#define _PCD_TOKEN_PcdS3BootScriptRuntimeTableReservePageNumber  134U
...
#define _PCD_TOKEN_PcdCpuStackSwitchExceptionList  251U
...
#define _PCD_TOKEN_PcdCpuKnownGoodStackSize  246U
  ...
  )
{
  ...
  Status = LockBoxDxeLibInitialize (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);

  Status = HobLibConstructor (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);

  Status = S3BootScriptLibInitialize (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);
  ...
  )
{
  ...
  Status = S3BootScriptLibDeinitialize (ImageHandle, SystemTable);
  ASSERT_EFI_ERROR (Status);
...
{
  return BootScriptExecutorEntryPoint (ImageHandle, SystemTable);
}
